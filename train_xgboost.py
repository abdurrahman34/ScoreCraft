...
try:
    ...
    for train_idx, val_idx in KFold(n_splits=8, shuffle=True, random_state=42).split(X_train_sample):
        X_train_fold = X_train_sample.iloc[train_idx]
        X_val_fold = X_train_sample.iloc[val_idx]
...
plt.title('XGBoost - Özellik Önemi (Top 20)')
plt.tight_layout()
...
print("\nXGBoost modeli eğitimi ve değerlendirmesi tamamlandı.")
